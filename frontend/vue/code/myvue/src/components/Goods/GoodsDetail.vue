<template>
  <div>
    ...
    <h1>{{ detail.title }}</h1>
    <p>
      市场价：<span><del>￥9999</del></span
      >销售价: <span style="color:red">￥{{ detail.price }}</span>
    </p>
    <p>库存：{{ detail.store }}</p>
    <p id="pNum">
      购买数量&nbsp;&nbsp;&nbsp;<span @click="minus">-</span
      ><span>{{ num }}</span
      ><span @click="add">+</span>
    </p>
    <div id="btnDiv">
      <mt-button type="primary" size="normal">立即购买</mt-button>
      <mt-button type="danger" size="normal" @click="showBall"
        >加入购物车</mt-button
      >
    </div>
    <div class="plainDiv">
      <mt-button type="primary" plain size="large" @click="toGoodsInfo"
        >图文介绍</mt-button
      >
